...
- Browser assertions live in `tests/*-browser.ts` modules (runtime, tap, motion, wrapping, wrap-intro, stop, overlap, seam-fill, preview, hints) orchestrated by `tests/browser-runner.ts`, which drives a headed Playwright browser against the production build. The stop suite (`tests/stop-browser.ts`) runs in both the full sweep and its own `STOP_ONLY=1` mode.

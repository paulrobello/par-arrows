...
- Progress, lives, failed-arrow history, and generator metadata are stored as a v6 logical save in browser `localStorage`. Generated catalogs are regenerated by a worker on reload.
...
Later blue/green two-ended arrows and yellow continuation edges have explicit engine boundaries and fixtures, but do not appear in the campaign. Non-cube content, undo, accounts, cloud saves, and custom audio are deferred. Hints, dark theme, and completion confetti remain available.

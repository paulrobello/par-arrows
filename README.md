...
- The first run plays cube 1 as an interactive walkthrough: it states the rules, has the player tap a blocked arrow — the collision costs one life and marks the arrow red — then guides clearing its blocker and the rest of the cube. Cubes 5, 11, 15, 20, and 25 open with their own one-time walkthroughs for parking, wrapping, shared-tail groups, chevron spots, and two-headed direction choice. While a walkthrough step expects an arrow, only that arrow (or its group) responds to taps; once its interactions finish, the remaining arrows clear in any order.
- The authored introductions at levels 1, 5, 11, 15, 20, and 25 have five lives. Runtime-generated levels are deterministic reverse-constructed puzzles, growing from 60 to 180 arrows early and then capped at 264 arrows, 26 × 26 cells per face, 40 cells per path, and a three-life floor.

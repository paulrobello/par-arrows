...
The owner rejected the remaining stamped S-curve repetition. Before another content revision, apply the [reference complexity criteria](docs/references/arrow-complexity-study.md): replace regular band templates with jointly composed irregular paths, measure normalized footprint repetition and run/turn patterns, and compare complete faces with the reference images. Treat visual variety as its own acceptance condition alongside solver and density checks. This study does not implement that revision.

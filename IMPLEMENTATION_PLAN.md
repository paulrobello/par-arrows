...
The owner superseded the fixed ten-level catalog: level 1 remains the authored teaching cube, levels 2–10, 12–14, and 16 onward are deterministic, solver-validated runtime puzzles derived from their logical level numbers, level 11 is an authored yellow-wrap teaching cube, and level 15 introduces overlapping tails. The same generated level number yields the same puzzle for all players and on retry. Progression has no final level. Path-following movement, body unwrapping across ordinary seams, permanent red failure history with one life penalty per arrow or connected group, free rotation with wheel/pinch zoom, right-angle paths with only validated shared tails, and rejection of self-contact remain unchanged. One arrow or connected tail group moves at a time, extra taps are ignored, rotation/zoom remain available, generated lives floor at three, and reload restores complete logical state. Press highlights selection; ambiguous taps do nothing.

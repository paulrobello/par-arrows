...
References 04–06 guide content toward densely packed mixtures of straight, hooked, zigzag, and winding multi-bend paths with varied lengths and face spans. Compare turn variety, spacing, readability, and wrapping with these photos while preserving the simple opening level and validating every route. Reference 07 documents the owner's yellow-edge head-continuation rule: animate the head turning onto the adjacent face with its body following. Keep that rule distinct from an existing body's ordinary-seam unwrapping. Level 11 is an authored, low-density introduction with one reciprocal yellow edge; generated levels from 12 onward (except the authored level 15) retain the established weighted edge distribution.
...
The owner superseded the fixed ten-level catalog: level 1 remains the authored teaching cube, levels 5, 11, 15, 20, 25, and 30 are authored mechanic introductions, and every other level is a deterministic, solver-validated runtime puzzle derived from its logical level number. The same generated level number yields the same puzzle for all players and on retry. Progression has no final level. Path-following movement, body unwrapping across ordinary seams, permanent red failure history with one life penalty per arrow or connected group, free rotation with wheel/pinch zoom, and right-angle paths with only validated shared tails remain unchanged; self-contact rejection was superseded on 2026-09-23 by the rule that arrows never collide with themselves. One arrow or connected tail group moves at a time, extra taps are ignored, rotation/zoom remain available, generated lives floor at three, and reload restores complete logical state. Press highlights selection; ambiguous taps do nothing.
...
Proposed canonical occupancy: a cell is an identified face-cell center; a link connects adjacent centers. A route is an ordered sequence of cells plus those links. Two different arrows may share only a continuous same-direction non-head tail segment in a validated group of two or three; point crossings, head overlap, and other shared cells or links are invalid. Within-face links join cardinal neighbors. A cross-face link joins the two distinct boundary cells through the seam, with a canonical identity independent of direction. Do not duplicate a crossing as two unrelated links or merge neighboring face cells into one cell. Edge lanes avoid physical cube vertices, so no three-face corner choice arises in MVP content.
...
Arrows never collide with themselves: occupancy comes from other arrows only, so a head that a spot sends back the way it came travels over its own body. Validation instead rejects any level where an arrow could loop forever, detected by a repeated cell-and-heading key, and any stop circle that would park an arrow folded back over its own body. A loop discovered in play is a content error and never an ordinary life penalty.

Future continuation must have a finite simulation budget derived from validated content bounds and cycle detection that accounts for relevant moving-body state. A repeated head face/position alone is not always a repeated full state. Surface loops need tests. An exhausted internal safeguard produces a diagnosable content error, not an arbitrary player penalty.

For overlapping tails, derive connected groups from shared directed non-head links and validate each member's independent full route against its peers. The simulation returns an aggregate result plus each member's trace. Ignore peer occupancy only inside a validated group. Any outside collision blocks the entire attempt; successful attempts remove all members in one revision. The solver and reverse-construction certificate clear each group atomically.

### A4: Attempt and level controller

Use explicit states for ready, animating exit, animating rebound, failed, and complete. Each accepted attempt owns a unique token and one immutable result. On collision, check the arrow or group failure history: charge one life and record every member ID only on the first failure, otherwise charge zero. Both cases still animate contact and rebound. Red stays until removal. Derive the final logical snapshot before presentation and persist according to Q11. Reveal any life loss at impact and completion after the last exit, without allowing duplicate callbacks to repeat either.
...
For a linked group, animate each member's own route using one shared world-space travel distance. Successful groups use the longest exit distance; blocked groups turn around at the earliest member contact. Extend short members' flight tracks as needed so all keep moving at the same speed. Restore every ribbon on settlement and highlight every member on press or hint.

### A6: Pointer and interface controller

Use one pointer gesture state machine: pending tap, orbit drag, pinch/zoom, and cancelled. On primary pointerdown, capture the unambiguous arrow under the pointer. A matching primary pointerup activates that captured arrow only if displacement stays below the 9 CSS-pixel implementation-default threshold and no drag, zoom, pinch, cancellation, lost capture, or blur occurred; release may land on whitespace or a neighboring narrow target. Blank or ambiguous downs capture nothing. Reaching the threshold rejects the tap even if the pointer returns to its start or intermediate move events were absent. Wheel, pinch, cancellation, lost capture, and blur cancel a pending tap. Show press highlight only when the app can start movement, and remove it while loading, errored, in the demo, or another arrow is moving. The one-active-attempt rule remains; orbit and zoom remain available during motion. Support mouse-wheel zoom, normalize wheel deltas, clamp near/far zoom, and prevent zoom gestures from activating arrows. Convert pointer coordinates using the canvas's actual rectangle after device rotation, zoom, and safe-area layout changes. Suppress the synthetic click that can follow a touch activation.
...
The production campaign imports authored levels 1, 5, 11, 15, 20, 25, and 30; it generates levels 2–4, 6–10, 12–14, 16–24, and 26–29 through `content/procedural.ts`. Levels 2–4 keep their literal version-1 seeds and unchanged geometry; levels 6–10 keep their version-4 runtime seeds. Generator version 5 derives seeds from level 12 upward; the authored cubes keep pinned versioned seeds (`:stop-intro:1`, `:wrap-intro:1`, `:overlap-intro:1`, `:directional-intro:1`, `:double-intro:1`, `:flip-intro:1`), and generated cubes 12–14 took new layouts in the v5 density bump. Changed attempts from level 5 onward restart while preserving unlocks and tutorial completion. The authored level 11 is a 4 × 4 cube with six short arrows, one per face, five lives, and a guaranteed reciprocal front-west/left-east yellow edge. Two safe moves wrap across that edge; the other four exit normally. It is exempt from random edge-count selection and generated density. Level 15 is an authored 4 × 4 cube with a linked pair, a linked trio, and an outside blocker. Generated layouts from level 16 use the existing weighted edge curve and insert validated pairs/trios before ordinary reverse-constructed arrows. The generator reverse-constructs free surface paths, validates occupancy/loop-freedom/solvability, and returns only a puzzle matching the configured bounds: early growth from 60 to 180 arrows, then a 264-arrow cap, 26 × 26 cells per face, and 40 cells per path. Each selected physical wrapping seam gets a 5/6/7-cell wrapping starter, and its full head-exit ray is protected during construction. Random target body lengths scale to 90/80/70% for one/two/three physical wrapping seams. `campaign-layouts.ts` and `levels.ts` and the offline generator remain legacy test fixtures only and are excluded from production imports.
...
Validate paths, occupancy, seams, arrow endpoints, mechanic versions, life budgets, absence of any possible continuation loop, and solution reachability. Use the same movement implementation as runtime. Run loop checks independently of current blockers. Save complete successful removal sequences as reproducible evidence. Exhaustive state enumeration is appropriate for small fixtures, while larger levels need a validated solver strategy and search bounds.

Persist progress through browser `localStorage` as schema v8. Store unlocked levels, current level, generator version and seed, remaining arrows, failed-arrow IDs, and lives, without a generated catalog. Store reduced-motion and System/Light/Dark settings separately so appearance changes preserve the active attempt. Write each accepted move's final logical result before its animation, and write Retry/reset and completion/unlock changes when they occur.

Stable attempts through level 4 resume exactly when their seeds match, as do the authored cubes 11 and 15. Changed attempts from level 5 onward regenerate the current layout for the same logical ID while retaining unlocks and tutorial completion. New saves use content version 8 and generator version 5. Failed groups count once for lives and once for accepted failure revisions; partial group removals or failure history are invalid. Restoration is asynchronous and storage normalization remains pure; only the app commits an accepted restored state.
...
- P2.3: Reject possible continuation loops (self-contact no longer applies since arrows pass over their own bodies). The approved just-vacated-tail boundary, seam occupancy, malformed data, and large travel distances have explicit tests.
...
Deliverables: deterministic runtime generator and worker loader, validator/solver evidence, a small-cube demo showing a failed touch followed by a successful touch, authored level 11 yellow-wrap and level 15 overlapping-tail introductions, endless sequential unlocks, replay, versioned `localStorage` saves with automatic resume, and PWA manifest/icons/install guidance. Level 1 remains authored; runtime generation begins at level 2 with authored introductions at levels 11 and 15.
...
- P5.2: The progression curve is observable: 60–180 arrows early, then no more than 264 arrows, 26 × 26 cells per face, and 40 cells per path. Level 1 and the onboarding demo retain their simple layouts. A repeated level number yields the same puzzle for every player and retry.
...
Development level previews use `?level=`, `?feature=wrap`, `?feature=overlap`, or `?wraps=0|1|2|3`; feature aliases `wrapping` and `wraparound` are case-insensitive. A combined level and filter searches at or after that level, with at most 1,000 candidates checked through edge-count metadata. Valid levels range from 1 to `Number.MAX_SAFE_INTEGER - 1`. Any of these selectors starts a separate preview that skips onboarding and never writes or deletes campaign storage during play, Retry, Next, Reset, or errors; Return to campaign exits preview. Go and Next preserve filters and write the resolved level back to the URL. Strict numeric parsing rejects duplicates, conflicts, and unknown features with a clear error. Bare `?test=1` continues to enable automation hooks only. Verification is recorded in progress.md.
...
| P8 | Implemented and locally verified 2026-09-15 | Yellow continuation rules, visible whole-edge markings, reciprocal seam direction changes, repeated crossings, cycle rejection, and solver-validated generated content. The authored level 11 introduction precedes generated yellow-edge layouts from level 12 (except the authored level 15); edge-count weights are documented in the PRD. |
...
| T5 | Continuation loops, loops hidden behind another arrow, and tail vacating. | Invalid levels rejected before play; no runtime life charge for invalid content; vacated-tail boundary tested in logical update order and continuous motion. |
...
- **K1 — Remaining movement details:** Path following is confirmed, and arrows never collide with themselves. The exact tail-vacating boundary still constrains valid content and the solver.
